    ...
    return (
      <div className="flex flex-col items-center gap-1 py-0.5">
        <div className="flex items-center gap-3 w-full">
          ...
          <span className="text-[10px] font-bold text-stone-400 shrink-0">사회자</span>
          <div className="flex-1 h-px bg-stone-200/60" />
        </div>
        <p className="text-[12px] font-medium text-stone-500 leading-relaxed text-center max-w-[92%] px-2 py-1">
          {log.text}
        </p>
      </div>
    );
  ...
  return (
    <div className={`flex w-full ${isMine ? 'justify-end' : 'justify-start'}`}>
      <div className={`flex items-start gap-2 max-w-[88%] ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
        <div className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[13px] font-extrabold ${avatarTone}`}>
          {speakerShortLabel}
        </div>
        <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
          {/* 말풍선 */}
          <div className={`relative px-4 py-3 text-[13px] leading-relaxed shadow-[0_8px_20px_rgba(0,0,0,0.03)] ${bubbleTone}`}>
            {/* 상단 배지 행: 유형 + 논증 구조 */}
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${typeBadgeTone}`}>
                {log.type}
              </span>
              <span className={`text-[10px] font-semibold ${bubbleMetaTone}`}>
