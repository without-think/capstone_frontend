...
const StanceView = ({ userStance, setUserStance, userIntensity, setUserIntensity, visible }) => {
  ...
  const color = userStance === 'pro' ? '#3b82f6' : userStance === 'con' ? '#f87171' : '#78716c';
  ...
  return (
    ...
    `}>
      ...
      <div className="w-full flex items-center justify-center gap-0">
        ...
        {/* 크기가 다른 원 선택 */}
        <div className="shrink-0 flex flex-col items-center gap-2 transition-all duration-500 opacity-100 translate-x-0">
                  ...
                  className={`rounded-full flex items-center justify-center font-extrabold transition-all duration-300
                    ${selected
                      ? userStance === 'pro'
                        ...
                        : 'bg-stone-700 text-white shadow-lg ring-4 ring-stone-200 scale-110'
